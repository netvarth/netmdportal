/**
 * NetMdResetPwdEmailPerformHandler.java
 *
 * Apr 04, 2013
 *
...
public class NetMdResetPwdEmailPerformHandler implements
	...
	public void sendMsgPerformed(SendMailMsgObj obj) {
		...
		if (obj.getStatus().equals(SendMsgStatus.SUCCESS)) {
			System.out.println("Successfully send  reset password email to netmd user  - CALLBACK FUNCTION");
		} else if (obj.getStatus().equals(SendMsgStatus.FAILED)) {
			System.out.println("Failed to send reset password email to netmd user  - CALLBACK FUNCTION");
		}
	}
}
