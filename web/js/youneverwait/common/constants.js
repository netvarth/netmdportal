constant_NetLimsEntry_Url="/youNeverWait/js/youneverwait/netlims/netlimsEntryPoint.js";
constant_NetLimsFunctions_Url ="/youNeverWait/js/youneverwait/netlims/netlimsFunctions.js";
constants_newNetLimsJson='/youNeverWait/json/new/newNetLims.json';
constants_adminChangePwdJsonas='/youNeverWait/json/changepassword/adminChangePassword.json';
constants_userLogJson='/youNeverWait/json/userlog/userlog.json';
constants_netlimsHealthmonitorJson='/youNeverWait/json/view/healthMonitor.json';
constants_graphHealthmonitorJson='/youNeverWait/json/graphHealth.json';
constants_newNetLims="/youNeverWait/js/youneverwait/netlims/new/newNetLims.js";
constant_Healthmonitor_Url="/youNeverWait/js/youneverwait/netlims/branch/branchHealthmonitor.js";
constant_NetLimsList_Msg="NETLIMS LIST";
constants_testDetails = "Test Details";
constants_nameRequired = "Name required";
constants_rateRequired = "Rate required";
constants_avoidDoubleQuotes = "Please avoid double quotes";
constants_testDeleteSuccess = "Test deleted successfully";

constants_ViewNetLimsEntryPoint="/youNeverWait/js/youneverwait/netlims/view/viewNetLimsEntryPoint.js";
constants_ViewNetLimsFunction="/youNeverWait/js/youneverwait/netlims/view/viewNetLimsFunctions.js";
constants_ViewNetLimsAction="/youNeverWait/js/youneverwait/netlims/view/viewNetLimsActions.js";
constants_ViewNetLimsValidation="/youNeverWait/js/youneverwait/netlims/view/viewNetLimsValidations.js";
constant_NetLimsView_Msg="View NetLims";

/*patient constants*/
constants_patientChangePwdJsonas='/youNeverWait/json/changepassword/patientChangePassword.json';
constant_patientChangePassword_Url="/youNeverWait/ynw/auth/changePassword";
constant_patientAppointmentFunctions_Url='/youNeverWait/js/appointment/appointmentFunctions.js';
constant_patientappointmentEntry_Url='/youNeverWait/js/appointment/appointmentEntry.js';
constant_index_Url='/youNeverWait/js/youneverwait/patientindex.js';
constants_newappointmentJson='/youNeverWait/json/new/newAppointment.json';
constants_viewappointmentJson='/youNeverWait/json/view/viewAppointment.json';

constant_NetMdEntry_Url="/youNeverWait/js/youneverwait/netmd/netmdEntryPoint.js";
constant_SettingsEntry_Url="/youNeverWait/js/youneverwait/settings/adminChangePwd.js";
constant_NetMdList_Msg="NETMD LIST";
constant_NetMdFunctions_Url="/youNeverWait/js/youneverwait/netmd/netmdFunctions.js";
constants_newNetMdJson='/youNeverWait/json/new/newNetMd.json';
constants_newNetMd="/youNeverWait/js/youneverwait/netmd/new/newNetMd.js";
constants_newSetsycJson="/youNeverWait/json/new/newsetSync.json";
constants_newsynctype="/youNeverWait/js/youneverwait/setSync.js";


constants_ViewMdEntryPoint="/youNeverWait/js/youneverwait/netmd/view/viewNetMdEntryPoint.js";
constants_ViewNetMdAction="/youNeverWait/js/youneverwait/netmd/view/viewNetMdActions.js";
constants_ViewNetMdFunctions="/youNeverWait/js/youneverwait/netmd/view/viewNetMdFunction.js";
constants_ViewNetMdValidations="/youNeverWait/js/youneverwait/netmd/view/viewNetMdValidations.js";
constant_NetMdView_Msg="View NetMd";

/*SYNCHRONISATION*/
constant_NetLimsSyncSet_Url="/youNeverWait/js/youneverwait/netlims/netlimsSycSet.js";
constant_NetLimsAccSyncSet_Url="/youNeverWait/js/youneverwait/netlimsAcc/netlimsAccBranchSynSet.js";
constants_netlimsSycSetJson="/youNeverWait/json/new/netlimssetSync.json"
constants_netlimsAccSycSetJson="/youNeverWait/json/new/netlimsAccsetSync.json"
constants_netlimsBranchSycSetJson="/youNeverWait/json/new/netlimsBranchsetSync.json";
constant_NetLimsBranchSyncSet_Url="/youNeverWait/js/youneverwait/netlims/branch/netlimsBranchSycSet.js"
constants_netmdSycSetJson="/youNeverWait/json/new/netmdsetSync.json";
constant_NetmdSyncSet_Url="/youNeverWait/js/youneverwait/netmd/netlmdSycSet.js";
constants_netmdAccBranchSycSetJson="/youNeverWait/json/new/netmdAccBranchsetSync.json";
constant_NetmdAccSyncSet_Url="/youNeverWait/js/youneverwait/netmdAcc/netmdAccBranchSynSet.js";
constants_netmdBranchSycSetJson="/youNeverWait/json/new/netmdBranchsetSync.json";
constant_NetmdBranchSyncSet_Url="/youNeverWait/js/youneverwait/netmd/branch/netmdBranchSycSet.js"
constants_netrxSycSetJson="/youNeverWait/json/new/netrxsetSync.json";
constant_NetrxSyncSet_Url="/youNeverWait/js/youneverwait/netrx/netrxSycset.js";
constants_netrxBranchSycSetJson="/youNeverWait/json/new/netrxBranchsetSync.json";
constants_netrxAccBranchSycSetJson="/youNeverWait/json/new/netrxAccBranchsetSync.json";
constant_NetrxBranchSyncSet_Url="/youNeverWait/js/youneverwait/netrx/branch/netrxBranchSycSet.js";
constant_NetrxAccBranchSyncSet_Url="/youNeverWait/js/youneverwait/netrxAcc/netrxAccBranchSycSet.js";

constant_NetLimsBranchEntry_Url="/youNeverWait/js/youneverwait/netlims/branch/branchEntryPoint.js";
constants_newNetLimsBranch="/youNeverWait/js/youneverwait/netlims/new/newNetLimsBranch.js";
constants_newNetMdBranch="/youNeverWait/js/youneverwait/netmd/new/newNetMdBranch.js";
constants_newNetBranchJson='/youNeverWait/json/new/newBranch.json';
constant_NetLimsBranchFunctions_Url="/youNeverWait/js/youneverwait/netlims/branch/branchFunctions.js";

constant_NetMdBranchEntry_Url="/youNeverWait/js/youneverwait/netmd/branch/branchNetmdEntryPoint.js";
constant_NetMdBranchFunctions_Url="/youNeverWait/js/youneverwait/netmd/branch/branchNetmdFunctions.js";

constants_ViewBranchEntryPoint="/youNeverWait/js/youneverwait/netlims/branchview/viewBranchEntryPoint.js";
constants_ViewBranchFunction="/youNeverWait/js/youneverwait/netlims/branchview/viewBranchFunctions.js";
constants_ViewBranchAction="/youNeverWait/js/youneverwait/netlims/branchview/viewBranchActions.js";
constants_ViewBranchValidation="/youNeverWait/js/youneverwait/netlims/branchview/viewBranchValidations.js";
constant_BranchView_Msg="View NetLims Branch";
constant_BranchList_Msg="BRANCH LIST";


constants_ViewNetmdBranchEntryPoint="/youNeverWait/js/youneverwait/netmd/branchview/viewNetmdBranchEntryPoint.js";
constants_ViewNetmdBranchFunction="/youNeverWait/js/youneverwait/netmd/branchview/viewNetmdBranchFunctions.js";
constants_ViewNetmdBranchAction="/youNeverWait/js/youneverwait/netmd/branchview/viewNetmdBranchActions.js";
constants_ViewNetmdBranchValidation="/youNeverWait/js/youneverwait/netmd/branchview/viewNetmdBranchValidations.js";
constant_NetmdBranchView_Msg="View NetMd Branch";

constants_newNetMDBranchJson='/youNeverWait/json/new/newNetMdBranch.json';
constant_BranchNetMdList_Msg="BRANCH LIST";
constants_newNetUserJson='/youNeverWait/json/new/newUser.json';

constants_newNetLimsUser="/youNeverWait/js/youneverwait/netlims/new/newNetLimsUser.js";


constant_newnetlimsBrchSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setBranchSync";
constant_newnetlimsAccBrchSync_Create_Url="/youNeverWait/ws/ui/lab/setBranchSync";
constant_newnetlimsSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setLabSync";
constant_newnetmdSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setNetMdSync";
constant_newnetRxSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setNetRxSync";
constant_newnetmdBrchSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setNetMdBranchSync";
constant_newnetmdAccBrchSync_Create_Url="/youNeverWait/ws/ui/netMd/setNetMdBranchSync";
constant_newnetRxBrchSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setNetRxBranchSync";
constant_newnetRxAccBrchSync_Create_Url="/youNeverWait/ws/ui/netRx/setNetRxBranchSync";
constants_SyncintervalRequired="Synchronisation Interval Required";
constant_newSync_Create_Url="/youNeverWait/ws/ui/superAdmin/setSync";
constant_newNetLims_Create_Url="/youNeverWait/ws/ui/superAdmin/createLab";
constant_newNetLimsBranch_Create_Url="/youNeverWait/ws/ui/superAdmin/createBranch";

constant_newNetMdBranch_Create_Url="/youNeverWait/ws/ui/superAdmin/createNetMdBranch";

constant_newNetLimsBranch_Delete_Url="/youNeverWait/ws/ui/superAdmin/deleteBranch";

constant_newNetMd_Create_Url="/youNeverWait/ws/ui/superAdmin/createNetMd";

constant_adminChangePassword_Url="/youNeverWait/ws/ui/superAdmin/changePassword";
constant_userLogControl_Url="/youNeverWait/ws/ui/superAdmin/enableLog";
constants_SuperadShowOrdersModalJson='/youNeverWait/json/BranchSuperadNetlimsOrders.json';
constants_SuperadBillListModalJson='/youNeverWait/json/BranchBillList.json';
constants_netmdBranchAccBillListModalJson='/youNeverWait/json/BranchAccBillList.json';

//new netlims validation
constants_organizationNameRequired="Organization Name Required";
constants_headofficeNameRequired="Head Office Name Required";
constants_headofficeEmailRequired="Head Office Email Required";
constants_ownerNameRequired="Owner First Name Required";
constants_ownerEmailRequired="Owner Email Required";
constants_userNameRequired="UseName Required";
constants_passwordRequired="Password Required";
constants_confirmpasswordRequired="Confirm Password Required"; 
constants_organizationnameInvalidMessage="Enter Valid Organization Name";
constants_headofficenameInvalidMessage="Enter Valid HeadOffice Name";
constants_headofficephoneInvalidMessage="Enter Valid Phone Number eg:04872535353";
constants_headofficemobileInvalidMessage="Enter Valid Mobile Number"; 
constants_headofficeemailInvalidMessage="Enter Valid Email";
constants_ownernameInvalidMessage="Enter Valid Owner First Name"; 
constants_ownerlastnameInvalidMessage="Enter Valid Owner Last Name";
constants_owneremailInvalidMessage="Enter Valid Owner Email";
constants_ownermobileInvalidMessage="Enter Valid Owner Mobile";
constants_ownerphoneInvalidMessage="Enter Valid Phone Number eg:04872535353";
constants_passwordMismatch="password do not match";


//new branch validation
constants_branchNameRequired="Branch Name Required";
constants_branchcodeRequired="Branch Code Required";
constants_emailRequired="Email Required";
constants_phoneRequired="Phone Required";
constants_mobileRequired="Mobile Required";
constants_phoneInvalid="Enter Valid Phone Number eg:04872535353";
constants_mobileInvalid="Enter Valid Mobile Number";
constants_emailInvalid="Enter Valid email";
constants_nodevicesRequired="Number of devices Required";


//new user validation
constants_userNameRequired="User Name Required";
constants_usernameInvalidMessage="Enter Valid User Name";


//change password validation
constants_userNameRequired="User Name Required";
constants_oldPasswordRequired="Old Password Required"
constants_newPasswordRequired="New Password Required"
constants_confirmPasswordRequired="Confirm Password Required"
constants_userNameInvalidMessage="Enter Valid User Name"


//Netlims Account
/* constant_NetLimsAccEntry_Url="/youNeverWait/js/youneverwait/netlimsAcc/netlimsAccEntryPoint.js";
constant_NetLimsAccbranchorder_Url="/youNeverWait/js/youneverwait/netlimsAcc/netlimsAccBranchOrders.js";
constant_NetLimsAccFunctions_Url ="/youNeverWait/js/youneverwait/netlimsAcc/netlimsAccFunctions.js";
constant_NetLimsAccList_Msg="BRANCH LIST";
constant_NetLimsAccBrachorder_Msg="Today's Branch Orders";


constant_SettingsNetlimsAccEntry_Url="/youNeverWait/js/youneverwait/settings/netlimsAccChangePwd.js";
constants_netlimsAccChangePwdJson='/youNeverWait/json/changepassword/netlimsAccChangePassword.json';
constants_netlimsOrdertypeJson='/youNeverWait/json/OrderType.json';

constant_netlimsAccChangePassword_Url="/youNeverWait/ws/ui/lab/changePassword";
constant_netlimsAccOrdertype_Url="/youNeverWait/ws/ui/order/setOrderType";
constants_newBranchNetLimsAccJson='/youNeverWait/json/new/newBranchNetlimsAcc.json';
constants_newNetLimsAccBranch="/youNeverWait/js/youneverwait/netlimsAcc/Branch/new/newNetLimsAccBranch.js";

constant_newNetLimsAccBranch_Create_Url="/youNeverWait/ws/ui/lab/createBranch";

constants_ViewNetlimsAccBranchEntryPoint="/youNeverWait/js/youneverwait/netlimsAcc/Branch/View/viewBranchNetlimsAccEntryPoint.js";
constants_ViewNetlimsAccBranchFunction="/youNeverWait/js/youneverwait/netlimsAcc/Branch/View/viewBranchNetlimsAccFunctions.js";
constants_ViewNetlimsAccBranchAction="/youNeverWait/js/youneverwait/netlimsAcc/Branch/View/viewBranchNetlimsAccActions.js";

constants_ViewNetlimsAccBranchValidation="/youNeverWait/js/youneverwait/netlimsAcc/Branch/View/viewBranchNetlimsAccValidations.js";

constant_BranchNetlimsAccView_Msg="View NetLims Branch";
constant_newNetLimsAccBranch_Delete_Url="/youNeverWait/ws/ui/lab/deleteBranch";
constants_ShowOrdersModalJson='/youNeverWait/json/BranchNetlimsOrders.json'; */
//NetMd Account
/* constant_netMdaccSettingsEntry_Url="/youNeverWait/js/youneverwait/settings/netMdAccChangePwd.js";
constants_netMdAccChangePwdJson='/youNeverWait/json/changepassword/netMdAccChangePassword.json';
constant_netMdAccChangePassword_Url="/youNeverWait/ws/ui/netMd/changePassword";
constant_NetMdAccEntry_Url="/youNeverWait/js/youneverwait/netmdAcc/netmdAccEntryPoint.js";
constant_NetMdAccList_Msg="BRANCH LIST";
constant_NetMdAccFunctions_Url="/youNeverWait/js/youneverwait/netmdAcc/netmdAccFunctions.js";
constants_newBranchNetMdAccJson='/youNeverWait/json/new/newBranchNetmdAcc.json';
constants_newNetmdAccBranch="/youNeverWait/js/youneverwait/netmdAcc/Branch/new/newNetmdAccBranch.js";
constant_newNetmdAccBranch_Create_Url="/youNeverWait/ws/ui/netMd/createNetMdBranch";
constants_ViewNetmdAccBranchEntryPoint="/youNeverWait/js/youneverwait/netmdAcc/Branch/View/viewBranchNetmdAccEntryPoint.js";
constants_ViewNetmdAccBranchFunction="/youNeverWait/js/youneverwait/netmdAcc/Branch/View/viewBranchNetmdAccFunctions.js";
constants_ViewNetmdAccBranchAction="/youNeverWait/js/youneverwait/netmdAcc/Branch/View/viewBranchNetmdAccActions.js";
constants_ViewNetmdAccBranchValidation="/youNeverWait/js/youneverwait/netmdAcc/Branch/View/viewBranchNetmdAccValidations.js";
constant_BranchNetmdAccView_Msg="View NetMd Branch";
constant_newNetmdAccBranch_Delete_Url="/youNeverWait/ws/ui/netMd/deleteNetMdBranch/"; */

//user log
constant_UserLogEntry_Url="/youNeverWait/js/youneverwait/userlog/userlogEntryPoint.js";
constant_UserLogFunctions_Url="/youNeverWait/js/youneverwait/userlog/userlogFunctions.js";
constant_UserLogList_Msg="USER LOG LIST";

//Sync Log
constant_SyncLogEntry_Url="/youNeverWait/js/youneverwait/syncLog/syncLogEntryPoint.js";
constant_SyncLogList_Msg="SYNC LOG LIST";
constant_SyncLogFunctions_Url="/youNeverWait/js/youneverwait/syncLog/syncLogFunctions.js";

//MacId Clear Confirm
constants_macIdConfirmJson='/youNeverWait/json/macidConfirm.json';
constants_netmdmacIdConfirmJson='/youNeverWait/json/netmdmacidConfirm.json';

//NetRx
constant_NetRxEntry_Url="/youNeverWait/js/youneverwait/netrx/netrxEntryPoint.js";
constant_NetRxList_Msg="NETRX LIST";
constant_NetRxFunctions_Url ="/youNeverWait/js/youneverwait/netrx/netrxFunctions.js";
constants_newNetRxJson='/youNeverWait/json/new/newNetRx.json';
constants_newNetRx="/youNeverWait/js/youneverwait/netrx/new/newNetRx.js";
constant_newNetRx_Create_Url="/youNeverWait/ws/ui/superAdmin/createNetRx";
constants_newNetRXBranchJson='/youNeverWait/json/new/newNetRXBranch.json';
constants_newNetRxBranch="/youNeverWait/js/youneverwait/netrx/new/newNetRxBranch.js";
constant_NetRxBranchEntry_Url="/youNeverWait/js/youneverwait/netrx/branch/branchNetrxEntryPoint.js";
constants_ViewRxEntryPoint="/youNeverWait/js/youneverwait/netrx/view/viewNetRxEntryPoint.js";
constant_newNetRxBranch_Create_Url="/youNeverWait/ws/ui/superAdmin/createNetRxBranch";
constants_ViewNetRxAction="/youNeverWait/js/youneverwait/netrx/view/viewNetRxActions.js";
constants_ViewNetRxFunctions="/youNeverWait/js/youneverwait/netrx/view/viewNetRxFunction.js";
constants_ViewNetRxValidations="/youNeverWait/js/youneverwait/netrx/view/viewNetRxValidations.js";
constant_NetRxView_Msg="View NetRx";
constant_NetRxBranchFunctions_Url="/youNeverWait/js/youneverwait/netrx/branch/branchNetrxFunctions.js";
constant_BranchNetRxList_Msg="BRANCH LIST";
constants_ViewNetrxBranchEntryPoint="/youNeverWait/js/youneverwait/netrx/branchview/viewNetrxBranchEntryPoint.js";
constants_ViewNetrxBranchFunction="/youNeverWait/js/youneverwait/netrx/branchview/viewNetrxBranchFunctions.js";
constants_ViewNetrxBranchAction="/youNeverWait/js/youneverwait/netrx/branchview/viewNetrxBranchActions.js";
constants_ViewNetrxBranchValidation="/youNeverWait/js/youneverwait/netrx/branchview/viewNetrxBranchValidations.js";
constant_NetrxBranchView_Msg="View NetRx Branch";

/* //NetRx Account
constant_NetRxAccFunctions_Url="/youNeverWait/js/youneverwait/netrxAcc/netrxAccFunctions.js";
constant_NetRxAccList_Msg="BRANCH LIST";
constant_NetRxAccEntry_Url="/youNeverWait/js/youneverwait/netrxAcc/netrxAccEntryPoint.js";
constants_newBranchNetrxAccJson='/youNeverWait/json/new/newBranchNetrxAcc.json';
constants_newNetrxAccBranch="/youNeverWait/js/youneverwait/netrxAcc/Branch/new/newNetrxAccBranch.js";
constant_newNetrxAccBranch_Delete_Url="/youNeverWait/ws/ui/netRx/deleteNetRxBranch/";
constant_newNetrxAccBranch_Create_Url="/youNeverWait/ws/ui/netRx/createNetRxBranch";
constants_ViewNetrxAccBranchEntryPoint="/youNeverWait/js/youneverwait/netrxAcc/Branch/View/viewBranchNetrxAccEntryPoint.js";
constants_ViewNetrxAccBranchFunction="/youNeverWait/js/youneverwait/netrxAcc/Branch/View/viewBranchNetrxAccFunctions.js";
constants_ViewNetrxAccBranchAction="/youNeverWait/js/youneverwait/netrxAcc/Branch/View/viewBranchNetrxAccActions.js";
constants_ViewNetrxAccBranchValidation="/youNeverWait/js/youneverwait/netrxAcc/Branch/View/viewBranchNetrxAccValidations.js";
constant_BranchNetrxAccView_Msg="View NetRx Branch";
constant_netRxaccSettingsEntry_Url="/youNeverWait/js/youneverwait/settings/netRxAccChangePwd.js";
constants_netRxAccChangePwdJson='/youNeverWait/json/changepassword/netRxAccChangePassword.json';
constant_netRxAccChangePassword_Url="/youNeverWait/ws/ui/netRx/changePassword"; */