	...
	this.result =	function(){ 
		...
		$j(result.values).each (function(index,data){
		   ...
		   if(mode=="print") {
				...
				var inputTag=$j('<input type="text"  id="APPT_' + index +'_1" class="'+data.className+'" value="' + data.FieldVal + '"/>');
			...
			if(data.time){
				 if(mode=="print") {
					...
					var inputTag=$j('<input type="text" class="'+data.time+'" value="'+data.unitVal+'" id="APPT_'+index+'_2"/>');
					spanTag.append(inputTag);
						if(data.UnitType)
